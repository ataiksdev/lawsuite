...
These routes are for YOU as the SaaS operator — not for your clients.
They require the requesting user to have the "admin" role AND their
org_id to match the PLATFORM_ADMIN_ORG_ID environment variable.

This prevents any client-org admin from accessing platform-wide data.

Endpoints:
  GET  /admin/organisations          — list all tenants
  GET  /admin/organisations/{id}     — tenant detail + subscription
  POST /admin/organisations/{id}/plan — manually override a plan
  GET  /admin/organisations/{id}/members — members of any tenant
  POST /admin/organisations/{id}/deactivate — suspend a tenant
  POST /admin/organisations/{id}/activate   — reactivate a tenant
...
async def require_platform_admin(current_user: AuthUser) -> CurrentUser:
    """
    Ensures the caller is both an org admin AND belongs to the
    designated platform admin organisation.

    Set PLATFORM_ADMIN_ORG_ID in .env to your own org's UUID.
    If not set, this guard blocks all requests (fail-safe default).
    """
